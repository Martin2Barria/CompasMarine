...
// CHANGED: Increased to allow downloading more documents faster
...
async function fetchAllControlDocPagesWithProfile(upstreamPath, credentials, extraParams = {}, paginationProfile) {
  ...
  while (keepFetching && nextPage <= CONTROL_DOC_MAX_PAGES) {
    const batchPages = createPageRange(nextPage, Math.min(nextPage + CONTROL_DOC_PAGE_CONCURRENCY - 1, CONTROL_DOC_MAX_PAGES));
  ...
  try {
    ...
    for (const entity of allEntities) {
      ...
      await dbPool.execute(`
        INSERT INTO entidades_api (external_id, nombre, rut, email, sexo, telefono, customer_id, entity_type_id, data_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
        nombre = COALESCE(?, nombre), rut = COALESCE(?, rut), email = COALESCE(?, email), sexo = COALESCE(?, sexo), telefono = COALESCE(?, telefono), data_json = ?, sincronizado_en = CURRENT_TIMESTAMP
      `, [external_id, nombre, rut, email, sexo, telefono, credentials.customerId, credentials.entityTypeId, jsonString, nombre, rut, email, sexo, telefono, jsonString]);
